 ...
 * @version 1.0
 */
public class Stock_Buscar extends JDialog implements Stock_IComando, KeyListener {
	...
	public Stock_Buscar() {
		buscarBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				datos = Stock_PanelBuilder.getDatosTabla();
				...
				if(!vectorBusqueda.isEmpty())
					Stock.statusBar.setText("Total Resultados encontrados: "+vectorBusqueda.size());
				else
					Stock.statusBar.setText("No se encontraron resultados");
	...
	public static void anterior() {
		if(lista.hasPrevious()) {
			...
			Stock_PanelBuilder.setCeldaSeleccionada(Integer.parseInt(s[0]), Integer.parseInt(s[1]));
		}
		actualizarBotones();		
	}
	...
	private static void actualizarBotones() {
		if(lista.hasPrevious())
			Stock_BarraHerramientasBuilder.setBusquedaAntEnabled(true);
		else
			Stock_BarraHerramientasBuilder.setBusquedaAntEnabled(false);

		if(lista.hasNext())
			Stock_BarraHerramientasBuilder.setBusquedaSigEnabled(true);
		else
			Stock_BarraHerramientasBuilder.setBusquedaSigEnabled(false);
	}
