 ...
 * @version 1.0
 */
public class Supplier_Buscar extends JDialog implements Supplier_IComando, KeyListener {
	 ...
	 *
	 */
	public Supplier_Buscar() {
		buscarBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				datos = Supplier_PanelBuilder.getDatosTabla();
				...
				if(!vectorBusqueda.isEmpty())
					Supplier.statusBar.setText("Total Resultados encontrados: "+vectorBusqueda.size());
				else
					Supplier.statusBar.setText("No se encontraron resultados");
	...
	public static void anterior() {
		if(lista.hasPrevious()) {
			...
			Supplier_PanelBuilder.setCeldaSeleccionada(Integer.parseInt(s[0]), Integer.parseInt(s[1]));
		}
		actualizarBotones();		
	}
	...
	private static void actualizarBotones() {
		if(lista.hasPrevious())
			Supplier_BarraHerramientasBuilder.setBusquedaAntEnabled(true);
		else
			Supplier_BarraHerramientasBuilder.setBusquedaAntEnabled(false);

		if(lista.hasNext())
			Supplier_BarraHerramientasBuilder.setBusquedaSigEnabled(true);
		else
			Supplier_BarraHerramientasBuilder.setBusquedaSigEnabled(false);
	}
