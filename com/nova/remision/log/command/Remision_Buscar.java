 ...
 * @version 1.0
 */
public class Remision_Buscar extends JDialog implements Remision_IComando, KeyListener {
	 ...
	 *
	 */
	public Remision_Buscar() {
		buscarBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				datos = Remision_PanelBuilder.getDatosTabla();
				...
				if(!vectorBusqueda.isEmpty())
					Remision.statusBar.setText("Total Resultados encontrados: "+vectorBusqueda.size());
				else
					Remision.statusBar.setText("No se encontraron resultados");
	...
	public static void anterior() {
		if(lista.hasPrevious()) {
			...
			Remision_PanelBuilder.setCeldaSeleccionada(Integer.parseInt(s[0]), Integer.parseInt(s[1]));
		}
		actualizarBotones();		
	}
	...
	private static void actualizarBotones() {
		if(lista.hasPrevious())
			Remision_BarraHerramientasBuilder.setBusquedaAntEnabled(true);
		else
			Remision_BarraHerramientasBuilder.setBusquedaAntEnabled(false);

		if(lista.hasNext())
			Remision_BarraHerramientasBuilder.setBusquedaSigEnabled(true);
		else
			Remision_BarraHerramientasBuilder.setBusquedaSigEnabled(false);
	}
